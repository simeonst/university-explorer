...
export default class Server {
    ...
    private static performQuery(req: restify.Request, res: restify.Response, next: restify.Next) {
        ...
        Server.insightFacade.performQuery(query).then((result) => {
            res.json(200, {result: result});
            return next();
        }).catch((err) => {
            res.json(400, {error: err.message});
    ...
    private static addDataset(req: restify.Request, res: restify.Response, next: restify.Next) {
        ...
        let raw: Buffer = Buffer.from(req.body);
        let b64 = raw.toString("base64");

        let params = req.params;

        let kind: InsightDatasetKind = params["kind"];
        let datasetID = params["id"];

        Server.insightFacade.addDataset(datasetID, b64, kind).then((result) => {
            res.json(200, {result: result});
            return next();
        }).catch((error) => {
